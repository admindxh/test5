...
public class ReadTibetController extends BaseController {
	...
	public ModelAndView cultural(String path) {
		if (StringUtils.isNotBlank(path)) {
			return culturalHandler(path);
		} else {
			ModelAndView modelAndView = (ModelAndView) CacheOperation
					.getInstance().getCacheData(this, "culturalHandler",
							new Object[] { path == null ? "" : path }, Constants.intervalTime, Constants.maxVisitCount);
			return modelAndView;
		}
	}
	...
	public ModelAndView culture(String type) {
		...
		if (culs.size() != 0) {
			addData(modelAndView, computeScore(culs.subList(0, 6)), "list1");
			addData(modelAndView, computeScore(culs.subList(6, 12)), "list2");
			addData(modelAndView, computeScore(culs.subList(12, 18)), "list3");
		}
		modelAndView.addObject("programNam", 2);
		return modelAndView;
	}
	...
	public List<Content> computeScore(List<Content> list){
	    for (Content content : list) {
	        Map others = content.getOthers();
	        if(others==null){
	            others = new HashMap();
	...
	public ModelAndView cultural() {
		...
		List<AdArea> ad = adAreaService
				.findbypro("78abc9785-123c-1s24-bab-0211056a05bcs");
	...
	public ModelAndView list(Pager pager, String type, String keywords,
			String orderType) {
		...
		if (pager.getDataList() != null && pager.getDataList().size() > 0) {
			for (Content c : (List<Content>) pager.getDataList()) {
				Pager page = new Pager();
				page.setPageSize(1);
				page = contentService.getReplyByPostCodeState(page,
						c.getCode(), null, 1);
	...
	@ResponseBody
	public String listJson(Pager pager, String type, String keywords,
			String orderType) {
		if (!StringUtil.isNotNull(type)) {
			type = Culture.getTypeId(Culture.TYPE_DYNAMIC);
		}
		...
		content.setContentTitle(keywords);
		contentService.searchOnOthers(pager, content, orderType);
		if (Culture.getMTypeId(Culture.TYPE_M_MUSIC).equals(
			...
			for (Content c : (List<Content>) pager.getDataList()) {
				...
				if (user != null) {
					c.setSortNum(userFavoriteService.isFavAlready(
							user.getCode(), c.getCode(),
							UserFavorite.User_Fav_Read) ? 1 : 0);
				} else {
					c.setSortNum(0);
				}
			}
	...
	public ModelAndView detail(String code) {
		...
		modelAndView.addObject("programNam", 2);
		modelAndView.addObject("isOpen",
				programaService.isOpenReply(Content.DETAIL_READ_TIBET_REPLY));
		return modelAndView;
	}

	@RequestMapping(value = "culture/preview")
	public ModelAndView previewinfo(Content content, String type,
	...
	public ModelAndView cultureDetail(String code) {
		...
		modelAndView.addObject("programNam", 2);
		modelAndView.addObject("isOpen", programaService
				.isOpenReply(Content.DETAIL_READ_TIBET_CULTURE_REPLY));
		return modelAndView;
	}
	...
	@ResponseBody
	public String loadOthersContent(Pager page, Content content,
			String orderType) {
		if (page.getPageSize() > 18) {
			return obj2json(page);
		}
		contentService.findOtherContent(page, content,
				content.getContentType(), orderType);
		return obj2json(page);
	}
	...
	public String gradeMusic(String code, String score) {
		...
		if (checkSessionOperate(session, name)) {
			...
			try {
				...
				map.put(Culture.FEILD_SCORE,
						grade(ps, (String) map.get(Culture.FEILD_RL_SCORE),
								score));
				contentService.updateOthers(code, map);
				content.setState(0);
			} catch (Exception e) {
	...
	private String grade(String ps, String rlps, String score) {
		try {
			...
			double countScore = (Double.valueOf(ps) * (countPeople - 1) + Double
					.parseDouble(score));
			...
			score = new DecimalFormat().format(b);
			score = score.substring(0, Math.min(score.length(), 1));
		} catch (Exception e) {
			e.printStackTrace();
		}
		return score;
	...
	/**
	 * 打分：总分加入分数，总人数加1 （分数格式 /d,/d） 逗号前是总分数,后面是总人数
	 * 
	 * @param oldScore
	 *            之前的分数 ,
	 * @param score
