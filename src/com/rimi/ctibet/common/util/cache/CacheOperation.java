...
import com.rimi.ctibet.common.util.http.HttpConnection;

/**
 * @author liyang
 * @version 1.0 创建时间：Apr 1, 2010 10:54:07 AM 类说明：缓存处理类
 */
public class CacheOperation {
	...
	private List<String> threadKeys;// 处于线程更新中的key值列表
	...
	private void addCacheData(String key, Object data, boolean check) {
		if (Runtime.getRuntime().freeMemory() < 5L * 1024L * 1024L) {// 虚拟机内存小于10兆，则清除缓存 5L*1024L*1024L
	...
	public Object getCacheData(Object o, String methodName, Object[] parameters, long intervalTime, int maxVisitCount) {
		StringBuffer key = this.getKey(o, methodName, parameters);
		...
		LOG.info(key + "缓存数据获取成功！");
		return cacheData.getData();
	}

	public StringBuffer getKey(Object o, String methodName, Object[] parameters) {
	...
	/**
	 * @author liyang
	 * @version 1.0 创建时间：Apr 1, 2010 10:57:24 AM 类说明：些类存放方法的主调对像,名称及参数数组
	 */
	...
	public CacheData removeCacheData(Object o, String methodName, Object[] parameters) {
		return cacheMap.remove(this.getKey(o, methodName, parameters));
	}
	...
	private final String cacheClearURL = "/web/cache/clear";

	public void removeAllRemoteCacheData() {
		try {
			Properties properties = new Properties();
			InputStream setting = this.getClass().getResourceAsStream("/commonSetting.properties");
			properties.load(setting);
			setting.close();
			String addresses = properties.getProperty("cache.machines");
			if (addresses != null && !addresses.trim().isEmpty()) {
				String[] hosts = addresses.split(",");
				for (String host : hosts) {
					host = host.trim();
					if (host.toLowerCase().startsWith("http")) {
						String url = host + cacheClearURL;
						Map<String, String> headers = new HashMap<String, String>();
						headers.put("Referer", url);
						byte[] data = HttpConnection.request(HttpConnection.RequestMethod.POST, url, headers ,null);
	...
	public static void main(String[] args) {
		getInstance().removeAllRemoteCacheData();
	}
