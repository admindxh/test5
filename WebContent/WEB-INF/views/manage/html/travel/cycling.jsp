...
<head>
    ...
    <title>游西藏-游西藏首页显示-骑行攻略管理</title>
            ...
            if (spanReg) {
                ...
                if (preview == 1) {
                    for (var i = 1; i <= 4; i++) {
                        $.ajax({
                            type: "post",
                            url: "${ctx }web/readTibetQxMgController/saveManageImg",
                    ...
                    window.location.href = "${ctx }web/readTibetQxMgController/list";
                    msgBox("保存成功！", "pass", 1600);
                } else {
                    for (var i = 1; i <= 4; i++) {
                        $.ajax({
                            type: "post",
                            url: "${ctx }web/readTibetQxMgController/previewManageImg",
                            data: $("#form" + i).serialize(),
                            dataType: "text",
                            async: false,
                            success: function (data) {
                    ...
                    var form = $('<form style="display" target="_blank" id="open" action="${ctx}web/readTibetQxMgController/previewManageFront" method="post"></form>');
                    $("body").append(form);
                    $("#open").submit();
                }
            }
...
<div class="main">
    ...
    <div class="location">
        <label>您当前位置为:</label>
        <span><a>骑行攻略管理</a> -</span>
        <span><a href="#" target="_self">骑行攻略管理</a></span>
    </div>
    <!-- 模块管理 { -->
