...
<head>
	...
	<title>游西藏-目的地管理-景点信息管理-景点修改</title>
...
<body>
	...
	<div class="main">
		...
		<div class="location">
			...
			<span><a href="#" target="_self">景点修改</a></span>
		</div>

		<!-- 模块管理 { -->
		<form id="scenic-edit-form" action="${ctx}web/viewController/editView" method="post">
			...
			<div>
				...
				<div class="filament_solid_ddd ov-au mt10 pos-rela">
					...
					<div class="formLine mt20">
						<div>
							...
							<div id="subDest" class="select-base">
								<input type="hidden" name="destinationCode" value="${view.destinationCode}">
						...
						<div class="posi_t30_r20">
							<div>
								<label>想去人数:</label>
								<input id="fakeWantCount" type="text" class="w-100 number" name="fakeWantCount" maxlength="7" value="${view.fakeWantCount}"/>
								<button type="button" class="btn-base" onclick="javascript:document.getElementById('fakeWantCount').value=${view.wantCount};">恢复默认数</button>
								<span class="txt-suggest ml10">*此项不填将显示真实人数</span>
							</div>

							<div class="mt40">
								<label>去过人数:</label>
								<input id="fakeGoneCount" type="text" class="w-100 number" name="fakeGoneCount" maxlength="7" value="${view.fakeGoneCount}"/>
								<button type="button" class="btn-base" onclick="javascript:document.getElementById('fakeGoneCount').value=${view.goneCount};">恢复默认数</button>
								<span class="txt-suggest ml10">*此项不填将显示真实人数</span>
							</div>
						</div>
					</div>
					...
					<div class="formLine mt20">
						<label>景点名称:</label>
						<input id="scenicName" type="text" class="w-260" name="viewName" value="${view.viewName}"/>
						<input id="scenicNameHidden" type="hidden" class="w-260" value="${view.viewName}"/>
						<span class="reqItem">*</span>
					</div>
					...
					<div class="formLine">
						...
						<div id="" class="disp-ib">
							...
							<div class="mb30">
								<img id="imgPreId" width="138" height="168" src="${ctx}${view.img}" />
								<input id="imgHiddenId" type="hidden" name="img" value="${not empty view.img?view.img:'/portal/static/default/square.png'}">
					...
					<div class="formLine">
						...
						<input id="hiddenId" type="hidden" name="viewImage" value="${view.viewImage}">
					</div>

					<div class="formLine">
						<label style="vertical-align: top;">主图缩略图:</label>
						<div id="preDiv" class="disp-ib">
							<c:forEach var="url" items="${view.viewImageArray }">
								<div class="mb30">
									<img class="scenImg" width="1000" height="360" src="${ctx}${url}" imgpath="${url}" />
					...
					<div class="formLine mt20">
						...
						<%-- <textarea name="viewIntroduce" id="scenicIntro" cols="70" rows="10" class="mb20">${view.viewIntroduce}</textarea> --%>
						<script id="scenicIntro" class="ueEditor_cont" name="viewIntroduce"  style="height:500px;" type="text/plain">${view.viewIntroduce}</script>
						<span class="reqItem">*</span>
					</div>
				</div>
			</div>
			...
			<div>
				...
				<div class="filament_solid_ddd ov-au mt30">
					<div class="formLine mt20">
						...
						<input type="text" class="w-260 toLink" name="view_360full" value="${view.view_360full}"/>
					</div>
					<div class="formLine mt20">
						<label class="w-160">景点高清全景链接:</label>
						<input type="text" class="w-260 toLink" name="hdFullUrl" value="${view.hdFullUrl}"/>
					</div>
					<div class="formLine mt20">
						<label class="w-160">景点实景视频链接:</label>
						<input type="text" class="w-260 toLink" name="realSceneVideoUrl" value="${view.realSceneVideoUrl}"/>
					</div>
					<div class="formLine mt20">
						<label class="w-160">景点地址:</label>
						<input id="address" type="text" class="w-260 secAdress" name="address" value="${view.address}"/>
						<input id="addressXY" type="hidden" class="w-260" name="xy" value="${view.xy}"/>
					...
					<div class="formLine mt20">
						...
						<script id="shortGuide" class="ueEditor_cont" name="shortGuide"  style="height:500px;" type="text/plain">${view.shortGuide}</script>
					</div>
					<div class="formLine mt20">
						<label class="pos_r_t5 w-160">导览:</label>
						<%-- <textarea name="guide" id="" cols="70" rows="10" class="mb20 secInfo">${view.guide}</textarea> --%>
						<script id="guide" class="ueEditor_cont" name="guide"  style="height:500px;" type="text/plain">${view.guide}</script>
					</div>
					<div class="formLine mt20">
						<label class="pos_r_t5 w-160">交通简介:</label>
						<!-- <textarea name="traffic" id="" cols="70" rows="5" class="mb20 secInfo"></textarea> -->
						<script id="shortTraffic" class="ueEditor_cont" name="shortTraffic"  style="height:500px;" type="text/plain">${view.shortTraffic}</script>
					</div>
					<div class="formLine mt20">
						<label class="pos_r_t5 w-160">交通:</label>
						<%-- <textarea name="traffic" id="" cols="70" rows="5" class="mb20 secInfo">${view.traffic}</textarea> --%>
						<script id="traffic" class="ueEditor_cont" name="traffic"  style="height:500px;" type="text/plain">${view.traffic}</script>
					</div>
					<div class="formLine mt20">
						<label class="pos_r_t5 w-160">注意事项简介:</label>
						<!-- <textarea name="notice" id="" cols="70" rows="5" class="mb20 secInfo"></textarea> -->
						<script id="shortNotice" class="ueEditor_cont" name="shortNotice"  style="height:500px;" type="text/plain">${view.shortNotice}</script>
					</div>
					<div class="formLine mt20">
						<label class="pos_r_t5 w-160">注意事项:</label>
						<%-- <textarea name="notice" id="" cols="70" rows="5" class="mb20 secInfo">${view.notice}</textarea> --%>
						<script id="notice" class="ueEditor_cont" name="notice"  style="height:500px;" type="text/plain">${view.notice}</script>
					</div>
				</div>
			</div>
			<!-- } 景点其他信息 -->
			...
			<div class="contClassify">
				...
				<div class="filament_solid_ddd ov-au mt30">
					<div class="formLine mt20 mb20">
						...
						<input type="text" maxlength="" class="w-320 seoInfo" name="tdkTitle" value="${view.tdkTitle }">
					</div>
					<div class="formLine mt20 mb20">
						<label class="w-180 ft-w-b ml40">&lt;Description&gt;标签:</label>
						<input type="text" maxlength="" class="w-320 seoInfo" name="tdkDescription" value="${view.tdkDescription }">
					</div>
					<div class="formLine mt20 mb20">
						<label class="w-180 ft-w-b ml40">&lt;Keywords&gt;标签(关键字):</label>
						<input type="text" maxlength="" class="w-320 seoInfo" name="tdkKeywords" value="${view.tdkKeywords }">
			...
			<div class="txt-ac mt40">
				<button class="btn-sure saveBtn" type="button">保存</button>
	...
	<script type="text/javascript"> 
		var ue = UE.getEditor('scenicIntro',{maximumWords:800});
	    ...
	    var ue_guide = UE.getEditor('guide',{maximumWords:2000});
	    ue_guide.addListener("contentChange", function(e) {
	    	checkLength(ue_guide, 'guide');
		});
	    
	    var ue_traffic = UE.getEditor('traffic',{maximumWords:2000});
	    ue_traffic.addListener("contentChange", function(e) {
	    	checkLength(ue_traffic, 'traffic');
		});
	    
	    var ue_notice = UE.getEditor('notice',{maximumWords:2000});
	...
	<script type="text/javascript">
		...
		$(".number").blur(function () {
			var $this = $(this),
					thisVal = $this.val();
			valid_txtBox_create(this, $.VLDTOR.IsNum(thisVal) && inputRange(this,1,7),'只能为1-7位正整数','top');
		});
		...
		$(".saveBtn").click(function () {
			...
			if(errMesgLen) {
				...
				if($("#scenicNameHidden").val()==$("#scenicName").val()){
					//msgBox("修改成功！", "pass", 2600);
					//$("#scenic-edit-form").submit();
					submitAndGetXy();
				}else{
