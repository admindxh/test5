...
<head>
    ...
    <title>游西藏-攻略管理-已审核的攻略-修改攻略</title>
...
<body>
    ...
    <div class="main">
		<form id="qxglId"  action="" method="post">
			<input value="${content.code }" name="code" type="hidden"/>
			<input value="${praise.code}" name="prcode" type="hidden"/>
			<!-- 页面位置-->
			<div class="location">
				<label>您当前位置为:</label>
				<span><a>游西藏</a> -</span>
				...
				<span><a href="#" target="_self">编辑攻略</a></span>
			</div>
			...
			<div class="muduleManage details filament_solid_ddd">
				...
				<div class="contClassify">
					...
					<div class="formLine">
						...
						<div id="theirMudle" class="select-base">
							...
							<dl>
								<c:forEach var="program" items="${programsList}">
									<dt   <c:if test="${program.code==content.programaCode}">class='selected'</c:if> inputValue="${program.code}" inputName="programaCode">${program.programaName}</dt>
								</c:forEach>
							</dl>
							<input id="proCode" type="hidden" value="${content.programaCode}" name="programaCode"/>
						</div>
						...
						<div  id="strategyType"  class="select-base">
							...
							<dl>
								<dt id="gl2" inputValue="1" inputName="isOfficial"  <c:if test="${1==content.isOfficial}">class='selected'</c:if>>官方发布</dt>
								<dt id="gl1" inputValue="0" inputName="isOfficial"  <c:if test="${0==content.isOfficial}">class='selected'</c:if>>用户上传</dt>
							</dl>
							<input id="isOfficial" type="hidden" value="${content.isOfficial}" name="isOfficial"/>
						</div>
						<span class="reqItem">*</span>
						<label>上传用户:</label>
						<input id="uploadAuthor"  type="text" maxlength="20" value="${content.authorCode }" name="authorCode" <c:if test="${1==content.isOfficial}">readonly="readonly"</c:if> >
						<span class="reqItem">*</span>
					</div>
					<div class="formLine">
						<label>相关目的地:</label>
						<div class="chooseDest formLine-middle-group">
								<c:forEach var="destination" items="${destinationListReal}">
									<span class="formEleGroup-squad">
										<input   value="${destination.code}" <c:if test="${destination.ischecked=='checked'}">class='des'</c:if> name="dest" type="checkbox">
										<label class="lbl_check">${destination.name}</label>
									</span>
								</c:forEach>
						</div>
					</div>
							...
							<div class="formLine-small-group hide">
								...
								<span class="scenic">
									<c:forEach  var="viewinfo" items="${desandvo.views}">
										<span class="formEleGroup-squad">
										<input name="vies"  <c:if test="${viewinfo[2]=='checked'}">checked='checked'</c:if> value="${viewinfo[1]}" type="checkbox">
				...
				<div class="contClassify">
					...
					<div class="formLine">
						<label>攻略标题:</label>
						<input id="strategyTitle"  type="text" name="contentTitle" maxlength="30" value="${content.contentTitle}" class="w-260">
						<span class="reqItem">*</span>
					</div>
				...
				<!-- 运营数据 { -->
				<div class="contClassify">
					<h2 class="title">运营数据<input type="hidden" name="praisecode"  value="${praise.code}"/></h2>
					
					<div class="formLine">
						<label   >收藏数:</label>
						<input  id="collect_num"  type="text" name="falseFavoriteNum" value="${praise.falseFavoriteNum}" maxlength="20">
						<a class="btn-base" onclick="javascript:change(this,${praise.favoriteNum})">恢复系统默认</a>
						<label >查看数:</label>
						<input  id="lookup_num" type="text" name="falseViewcount" value="${praise.falseViewcount}"  maxlength="20">
						<a class="btn-base" onclick="javascript:change(this,${praise.viewCount})">恢复系统默认</a>
						<label>被赞数:</label>
						<input id="praise_num"  type="text" name="falsePraise" value="${praise.falsePraise}"  maxlength="20">
						<a class="btn-base" onclick="javascript:change(this,${praise.truePraise})">恢复系统默认</a>
					</div>
				</div><!-- } 运营数据 -->
    ...
    <script type="text/javascript">
		...
		$("#issue").click(function() {
			...
			if(VLDRST.TheirMudle && VLDRST.StrategyType && VLDRST.UploadAuthor && VLDRST.DestScenic && VLDRST.StrategyTitle && VLDRST.Collect_num && VLDRST.Lookup_num && VLDRST.Lookup_num) {
				save('${ctx}/web/readTibetSgPassMgController/update','qxglId')
...
//			msgBox("请至少选择一个景点!", "info", 2200);
	...
	<script type="text/javascript">
	function change(va,trueValue){
		$(va).prev("input").val(trueValue);
	}
	</script>
    <!-- 利用空闲时间预加载指定页面 -->
