...
<head>
    ...
    <title>游西藏-游西藏首页显示-综合攻略管理</title>
	    ...
	    function submit(preview){
			...
			if(thisWebHasVal) {
				...
				for( var i = 0; i < linkInput_len; i++) {
					var thisToLink = linkInput.eq(i);
					thisToLink.blur();
				}
			} else {
				msgBox("至少需要输入一项内容！", "erro", 2600);
				return false;
			}
			...
			if(spanReg) {
				if(preview==1)
				{
					for(var i=1;i<=4;i++){
					  $.ajax( {
							type : "post",
							url : "${ctx }web/readTibetZhMgController/saveManageImg",
					 ...
					 window.location.href="${ctx }web/readTibetZhMgController/list";
					 msgBox("保存成功！", "pass", 1600);
			   }else{
				   for(var i=1;i<=4;i++){
						  $.ajax( {
								type : "post",
								url : "${ctx }web/readTibetZhMgController/previewManageImg",
								data : $("#form"+i).serialize(),
								dataType : "text",
								async : false,
								success : function(data) {
					...
					var  form  = '<form style="display" target="_blank" id="open" action="${ctx}web/readTibetZhMgController/previewManageFront" method="post"></form>';
					$("body").append(form);
					$("#open").submit();
				 }
...
<body>
   <div class="main">
			...
			<div class="location">
				<label>您当前位置为:</label>
				<span><a>游西藏首页显示</a> -</span>
				<span><a href="#" target="_self">综合攻略管理</a></span>
			</div>
			<!-- 模块管理 { -->
			<div class="muduleManage filament_solid_ddd">
				...
				<div class="show">
				 <!-- form1{ -->
				 <form action="${ctx }web/homeController/saveManageImg" method="post" enctype="multipart/form-data" id="form1">
					<div class="posidSet-unit">
						...
						<div class="right mt10 perc40">
							...
							<input type="hidden"  id="hiddenId1" class="url" name="url" 
                  ...
                  <!-- form2{ -->
				  <form action="${ctx }web/homeController/saveManageImg" method="post" enctype="multipart/form-data" id="form2">
					<div class="posidSet-unit">
						<i>图片2</i>
						<div class="left perc60">
							<div class="formLine">
						...
						<div class="right mt10 perc40">
							...
							<c:if test="${not  empty imageList[0][1].url }">src="${ctx}${imageList[0][1].url}" </c:if>
							 src="${ctxManage}/resources/img/ele/loadImg_default_d.jpg" title="缩略图名称" alt="请上传图片"
                            class="style-d" style="width: 180px;height: 138px;">
							<input type="hidden" class="url" id="hiddenId2" name="url" 
				 ...
				 <!-- form3{ -->
				  <form action="${ctx }web/homeController/saveManageImg" method="post" enctype="multipart/form-data" id="form3">
					<div class="posidSet-unit">
						<i>图片3</i>
						<div class="left perc60">
							<div class="formLine">
						...
						<div class="right mt10 perc40">
							...
							<input type="hidden" class="url" id="hiddenId3" name="url" 
                  ...
                  <!-- }form1 -->
                   <!-- form4{ -->
				  <form action="${ctx }web/homeController/saveManageImg" method="post" enctype="multipart/form-data" id="form4">
					<div class="posidSet-unit">
						<i>图片4</i>
						<div class="left perc60">
							<div class="formLine">
						...
						<div class="right mt10 perc40">
							...
							<c:if test="${ not  empty imageList[0][3].url }">src="${ctx}${imageList[0][3].url}" </c:if>
							    title="缩略图名称" alt="请上传图片" class="style-d" style="width: 210px;height: 115px;">
							<input type="hidden" class="url" id="hiddenId4" name="url" 
	...
	<script type="text/javascript">
		...
		$(document).on("blur", ".posidSet-unit .formLine input[type='text']", function() {
			...
			// 执行验证并设置验证结果的状态
			valid_txtBox_create(this,regUrl || regNull,'请输入以http(s)://开头的链接','right');
		});
	</script>
    <!-- 利用空闲时间预加载指定页面 -->
