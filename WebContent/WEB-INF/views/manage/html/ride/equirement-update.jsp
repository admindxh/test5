...
<body>
		...
		<form id="qxglId"  action="" method="post">
			...
			<div class="muduleManage details filament_solid_ddd">
				...
				<div class="contClassify">
					...
					<div class="formLine"  >
						...
						<div id="theirMudle"  class="select-base">
							...
							<input id="proType" type="hidden" value="${pcode}" name="proType"/>
						</div>
						<span class="reqItem">*</span>
						
						<label>价格:</label>
						<div  id="strategyType" class="select-base">
							<input id="collect_num"  type="text" value="${equipment.price }" name="price" maxlength="20">
						</div>
						<span class="reqItem">*</span>
						<span id="storeNum" ${equipment.payType == '2' ? "style='display: none;'" : ""}>
							<label>数量:</label>
							<input id="lookup_num"  type="text"  name="count" value="${equipment.payType == '2' ? '' : equipment.count }" maxlength="20">
							<span class="reqItem">*</span>
						</span>
					</div>
					<div class="formLine">
						<label>推荐首页:</label>
						<input  type="radio"
							<c:if test="${equipment.recommoned==1}">
							checked="checked"
						</c:if>
						 name="recommoned" checked="checked" value="1"  />是&nbsp;&nbsp;&nbsp;&nbsp;<input 
						 <c:if test="${equipment.recommoned==2}">
							checked="checked"
						</c:if>
						  type="radio" name="recommoned" value="2"  />否
					</div>
					<div class="formLine">
						<label>支付类型:</label>
						<div class="chooseDest formLine-middle-group">
								<input type="radio" 
									<c:if test="${equipment.payType==1}">
											checked="checked"
									</c:if>
								 class="cktype" checked="checked" name="payType" value="1" />本站支付<%--本站支付 --%>
								<span style="width: 30px;">&nbsp;</span><input 
								<c:if test="${equipment.payType==2}">
									checked="checked"
								</c:if>
								type="radio" class="cktype" name="payType" value="2" />其他站点支付 <%--其他站点支付 --%>
					...
					<div class="formLine">
						<label>简介:</label>
						<div class="chooseDest formLine-middle-group">
								<textarea rows="3"  cols="40"    id="summary" name="summary">${equipment.summary }</textarea>
								<span class="reqItem">*</span>
						</div>
					</div>
					<div class="formLine">
			                ...
			                <div class="formLine">
								<label class="w-120" style="vertical-align: top;">缩略图:</label>
								 ...
								 <img id="imgshow" class="desImg1" style="width:400px;height:300px;" 
								    <c:if test="${empty equipment.sumaryimg}">
								 		src="${ctxManage}/resources/img/ele/loadImg_default_a.jpg"
								 	</c:if>
								 	<c:if test="${not   empty equipment.sumaryimg}">
								 		src="${ctx }/${equipment.sumaryimg}"
								 	</c:if>
								  alt=""/>
								 <input id="sumaryimg" type="hidden" class="url" name="sumaryimg" value="${equipment.sumaryimg}">
							</div>	
				...
				<div class="contClassify">
					...
					<div class="formLine">
						<label>装备标题:</label>
						<input id="name" type="text" name="name" value="${equipment.name }" maxlength="30" class="w-260">
						<span class="reqItem">*</span>
					</div>
    ...
    <script type="text/javascript">
    	...
    	/** 是否其他站点支付. */
    	var isOtherSite = (function () {
    		if ($(":radio[name='payType']:checked").val() == "2") {
    			return true;
    		} else return false;
    	})();
    	...
    	$(document).on("click",".cktype",function(){
				...
				if(payType=="1"){
					$("#urlShow").html("");		
					 ...
					 $("#urlShow").html('其它站点支付路径:<input id="payurl" value="${equipment.payurl}" class="w-260"  type="text" name="payurl" maxlength="100"><span class="reqItem">*</span>');
		...
		$("#issue").click(function() {
			...
			if(numberError<=0) {
				save('${ctx}/web/equip/update','qxglId')
