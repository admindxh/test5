...
<head>
    ...
    <title>系统设置-运营管理-其他页面管理-编辑页面</title>
...
<div class="main">
    ...
    <div class="searchManage">
			<a href="javascript:void(0)" id="preview" class="btn-anchor">预览</a>
	</div>
    <!-- 模块管理 { -->
    <form id="member-add-form" action="otherWebedit" method="post">
	<!--     	隐藏属性    -->
	...
	<input type="hidden" name="code" value="${content.code}">
		<div class="filament_solid_ddd pos-rela mt30">
			<div class="formLine mt20">
				<label>文章标题:</label>
				<input name="contentTitle" value="${content.contentTitle }" <c:if test="${content.contentType eq '4020'}">readonly</c:if> id="articleTitle" type="text" class="w-320" />
				<span class="reqItem">*</span>
			</div>

			<div class="formLine">
				<label class="pos_r_t5">正文:</label>
				<script id="contents" class="ueEditor_cont" name="content"  style="height:500px;" type="text/plain">${content.content }</script>
			</div>
			...
			<div class="contClassify mt20">
				...
				<div class="formLine">
					...
					<input name="keywords" id="keywords" value="${content.keywords}" type="text" maxlength="" class="w-320">
		...
		<div class="formLine mt20 mb30">
			<div class="saveOper">
				<button id="saveForm" class="btn-sure sureModif mr60" type="button">保存</button>
				<button id="preview" onclick="javascript:back()" class="btn-sure ml20 mr200" type="button">取消</button>
			</div>
		</div>
    </form>
    <!-- } 模块管理 -->
...
<script type="text/javascript">
 $("#saveForm").on('click',function(){
       $("#member-add-form").attr('target','_self');
        $("#member-add-form").attr('action','otherWebedit.html');
...
<script type="text/javascript">
	function back(){
		...
		$(document).one('click', '.sure', function(){
			location.href='<%=basePath %>manage/html/settings/otherWeb'
			closePopup();
		});
	}
</script>
