        ...
        <div class="muduleManage details filament_solid_ddd">
            <div>
                ...
                <div class="formLine floatfix">
                    <label>帖子标题:</label>
                    <input type="text"  value="${c.contentTitle }" name="contentTitle" class="w-260">
                    <label>发布人:</label>
                    <input type="hidden" name="createuserCode"  value="${c.createuserCode}"/>
                    <input type="text"   name="authorCode" value="${c.authorCode }" class="w-260" >
                    <div class="disp-ib mt10">
                        <label>是否置顶:</label>
                        <%--<c:if test="${c.isTop==1}"></c:if>--%>
                        <input id="yes" <c:if test="${c.isTop==1}">checked="true"</c:if> type="radio"  name="isTop" value="1" class="ml50"><label for="yes" class="lbl_check">是</label>
                        <input id="no" <c:if test="${c.isTop==0}">checked="true"</c:if> type="radio" name="isTop" value="0" class="ml50" ><label for="no" class="lbl_check">否</label>
                    </div>
                    ...
                    <div class="formLine mt20">
                        ...
                        <div id="culType" class="select-base">
                            <i class="w-140">${programa.programaName}</i>
                            <dl>
                                <c:forEach var="plate" items="${plates}">
                                    <dt inputValue="${plate.code}" inputName="programaCode">${plate.programaName}</dt>
                                </c:forEach>
                            </dl>
				            <input id="programaCode" type="hidden" value="${programa.code }" name="programaCode"/>
                        </div>
                        <label class="ml140">浏览量:</label>
                        <input type="text" value="${p.falseViewcount }" name="viewCount" class="w-100  number"/>
                        <a href="#">恢复系统值</a>
                        <label class="ml20">赞:</label>
                        <input type="text" value="${p.falsePraise }" maxlength="7"  name="praiseCount" class="w-100 number"/>
                        <a href="#">恢复系统值</a>
                    </div>
                </div>
                <!-- } 文章标题 -->
				...
				<div>
					...
					<div class="filament_solid_ddd ov-au mt30">
						<div class="formLine mt20 mb20">
							...
							<input type="text" maxlength="" class="w-320 seoInfo" name="tdkTitle" value="${c.tdkTitle }">
						</div>
						<div class="formLine mt20 mb20">
							<label class="w-180 ft-w-b ml40">&lt;Description&gt;标签:</label>
							<input type="text" maxlength="" class="w-320 seoInfo" name="tdkDescription" value="${c.tdkDescription }">
						</div>
						<div class="formLine mt20 mb20">
							<label class="w-180 ft-w-b ml40">&lt;Keywords&gt;标签(关键字):</label>
							<input type="text" maxlength="" class="w-320 seoInfo" name="tdkKeywords" value="${c.tdkKeywords }">
...
//		});
	    ...
	    $(".number").blur(function () {
	        var this_val = $(this).val();
	        valid_txtBox_create(this, $.VLDTOR.IsNum(this_val), "只能为数字", "top")
	    });
	    /* SEO 验证 */
		...
		$("#saveForm").click(function() {
	        ...
	        if(regSpan){
	        	...
	        	ispost = true;
	        	save('${ctx}/web/passPostController/updatePost','creatRead');
				return false;
	        }
		});
		/* 保存验证 */
