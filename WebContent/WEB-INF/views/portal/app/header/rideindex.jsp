...
<div class="navbar cb-index-nav navbar-static-top" id="top" role="banner">
    <div class="container">
        <div class="navbar-header">
            <h1 class="logo">
                <a href="${ctx}" class="navbar-brand" style="width: 200px;"><img src="${ctxPortal }assets/icon/logo2.png" alt="logo" width="200" height="60"></a>
            </h1>
        </div>
    ...
    >
        ...
        <div class="container">
            <ul class="list-unstyled">
                <li>
                    <c:forEach items="${programaTwo}" var="program">
                    	<c:choose>
	                   		<c:when test="${ip eq '127.0.0.1' && program.programaName eq '装备推荐' }">
	                   			<c:set var="title_display" value=""/>
	                   		</c:when>
	                   		<c:when test="${ip eq '125.70.10.34' && program.programaName eq '装备推荐' }">
	                   			<c:set var="title_display" value=""/>
	                   		</c:when>
	                   		<c:when test="${ip eq '183.223.252.42' && program.programaName eq '装备推荐' }">
	                   			<c:set var="title_display" value=""/>
	                   		</c:when>
	                   		<c:when test="${program.programaName eq '装备推荐'}">
                        ...
                        <a style="${title_display}" href="${ctxIndex}/${program.programaUrl}" target="_blank">${program.programaName}</a>
                        <input type="hidden" value="${program.code}">
