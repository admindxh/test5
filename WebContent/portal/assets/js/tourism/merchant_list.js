    ...
    seajs.use(["jquery", "avalon", "paginator", 'slick/slick.js'], function ($, avalon) {
        ...
        function getAllActivityList(currentPage) {
            ...
            if (allActivityVM.$cacheData[currentPage]) {
                ...
                //正在加载中图片提示
                $(".bd-heading").append('<img id="load" style="display: block;padding-top: 100px;margin: 0 auto;" src="'+basePath_+'/portal/static/default/loading.gif">');
                
                $.post(basePath_+'tourism/merchant/avalonMerchantList', {
                    rule: rule_,
                    type: type_,
                    viewCode: viewCode_,
    ...
    seajs.use(['jquery', 'map'], function ($, myMap) {
        $(function(){
	        // 初始化景点地图
    ...
    function loadViewList(map){
		...
		$.post(basePath_+'tourism/view/searchViewList', param, function(response){
			map.setGeoData($('#region-menu>.text').text(), response.dataList)
		}, 'json');
	}
